...
Two documents carry work this one only points at. The graph store move is written out gate by gate in the [graph store migration runbook](2026-08-04-graph-store-migration-runbook.md), and the per-organisation instance work is designed in the [control plane design](2026-08-04-control-plane-design.md), which also holds the cost model. Neither is repeated here.
