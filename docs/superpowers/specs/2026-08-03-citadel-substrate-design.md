...
**Why it is one project with dataset scoping:** upstream declares PostgreSQL as one of the graph backends supporting multi-user partitioning. Moving the graph and turning on per-dataset context are the same change to the same call sites, and doing them separately means doing the work twice.
