...
def test_public_pages_share_one_top_nav() -> None:
    """/info, /partners, and /login carry the same nav, each marking its entry.
    ...
    for page in (info, partners, login):
        assert 'class="topnav"' in page
        assert '<a href="/info"' in page
        assert '<a href="/partners"' in page
        assert '<a href="/login"' in page
        assert "Sign in</a>" in page
    ...
    assert '<a href="/login" aria-current="page">' in login
