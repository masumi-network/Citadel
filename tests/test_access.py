...
def test_repeated_rejections_do_not_flood_the_audit_log(tmp_path: Path) -> None:
    """One client with a revoked token must not evict the whole audit ring.

    Observed in production: a stale capture hook retried a revoked token every
    couple of minutes, and every attempt appended an audit event. At that rate
    it fills max_audit_events in well under a day, so the audit log stops being
    a record of what happened on the node and becomes a log of one broken
    client.

    The first rejection is still audited, so "a revoked token is in use" is
    never lost, and the deploy log still carries every attempt.
    """
    access_store = store(tmp_path)
    created = access_store.create_principal_token(
        name="stale-hook", kind="service_account", role="writer"
    )
    ...
    events = rejection_events(access_store)
    assert len(events) == 1, f"50 retries wrote {len(events)} audit events"
    assert events[0]["detail"]["reason"] == "revoked"


def test_a_persistent_rejection_is_re_audited_after_the_interval(tmp_path: Path) -> None:
    """Suppression is a rate limit, not a mute.

    A token still being presented an hour later is worth a fresh event, or the
    audit log would say the problem stopped when it did not.
    """
    access_store = AccessStore(tmp_path / "access.json", rejection_audit_interval_seconds=0.0)
    created = access_store.create_principal_token(
        name="stale-hook", kind="service_account", role="writer"
    )
    access_store.revoke_token(created.api_token.id)
    ...
    for _ in range(3):
        access_store.authenticate_token(created.token)

    assert len(rejection_events(access_store)) == 3, "a zero interval must not suppress"


def test_one_noisy_token_does_not_mask_another(tmp_path: Path) -> None:
    """Suppression is per token, so a loud client cannot hide a second one.

    Keying the rate limit globally would let the hammering hook swallow the
    first sighting of a different revoked token, which is the event someone
    actually needs to see.
    """
    access_store = store(tmp_path)
    noisy = access_store.create_principal_token(
        name="noisy", kind="service_account", role="writer"
    )
    quiet = access_store.create_principal_token(
        name="quiet", kind="service_account", role="writer"
    )
    access_store.revoke_token(noisy.api_token.id)
    access_store.revoke_token(quiet.api_token.id)

    for _ in range(30):
        access_store.authenticate_token(noisy.token)
    access_store.authenticate_token(quiet.token)

    audited = {event["detail"]["token_id"] for event in rejection_events(access_store)}
    assert audited == {noisy.api_token.id, quiet.api_token.id}
