...
from kb.model_routing import (
    ...
    normalize_free_openrouter_model,
    route_for,
)
...
def test_direct_route_rejects_paid_override(monkeypatch) -> None:
    _clear_routing_env(monkeypatch)
    monkeypatch.setenv("CITADEL_LLM_MODEL_RESEARCH", "anthropic/claude-sonnet-4")

    assert route_for("research").model == "openrouter/free"


def test_direct_route_accepts_explicit_free_model(monkeypatch) -> None:
    _clear_routing_env(monkeypatch)
    monkeypatch.setenv(
        "CITADEL_LLM_MODEL_REASONING",
    ...
    assert route_for("reasoning").model == "qwen/qwen3-coder:free"


def test_cognee_route_replaces_paid_stage_overrides(monkeypatch) -> None:
    _clear_routing_env(monkeypatch)
    monkeypatch.setenv("LLM_MODEL", "openrouter/anthropic/claude-sonnet-4")
    monkeypatch.setenv("LLM_QUERY_MODEL", "qwen/qwen3-coder:free")
...
def test_disabled_router_still_rejects_paid_models(monkeypatch) -> None:
    _clear_routing_env(monkeypatch)
    monkeypatch.setenv("CITADEL_MODEL_ROUTING_ENABLED", "false")
    monkeypatch.setenv("CITADEL_LLM_MODEL", "anthropic/claude-sonnet-4")
    monkeypatch.setenv("LLM_MODEL", "openrouter/anthropic/claude-sonnet-4")

    assert route_for("reasoning").model == "openrouter/free"
    assert configure_cognee_model_routes()["LLM_MODEL"] == (
        DEFAULT_COGNEE_FREE_ROUTER_MODEL
    )


def test_free_model_normalization_rejects_auto_router() -> None:
    assert normalize_free_openrouter_model("openrouter/auto") is None
    assert normalize_free_openrouter_model("openrouter/openrouter/free") == (
        "openrouter/free"
    )
