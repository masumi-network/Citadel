...
Current phase: all five failing PR 256 CI lanes remediated and pushed as head `3ce3328` after adversarial review; awaiting remote CI, then final-image Docker gates at the new head
Current sprint: CITADEL-QDRANT-2026-08-09

## Active goal

- REPORTED: finish Citadel v0.5 as a portable Docker release candidate using Cognee `1.4.1`, Qdrant `1.19.0`, SQLite, Ladybug, and lifecycle v1. Prove deterministic seed, live GitHub Central, graph, capture, HTTP, CLI, MCP, shared traces, two-seat isolation, promotion, benchmark, restart, outage, stress, backup, and restore inside production Compose while following Citadel and Qdrant logs.
- CORRECTED 2026-08-10: the user explicitly expanded the goal to finish and ship the production release. Local Docker gates, independent reviews, sequential commits, PR integration, and the immutable v0.5 release path are now in scope. Railway mutation, credential rotation, production data deletion, and any release mechanism not recorded in the approved release contract remain out of scope.
- REPORTED 2026-08-10: the user approved deployment to the existing Citadel Railway project only after local core gates pass. VERIFIED read-only Railway inventory: active project `Citadel` has `Citadel-Archive`, `Citadel-GitHub-Sync`, and PostgreSQL but no Qdrant service; project `Citadel (qdrant)` has Qdrant and `@citadel/web` but reports a non-null `deletedAt`. Exact deployment target and topology remain unresolved. No Railway mutation occurred.
- VERIFIED: durable design is `docs/superpowers/specs/2026-08-09-citadel-v050-docker-release-design.md`.

## Completed
- [x] CITADEL-RELEASE-PROBE-01, owner: implementer. VERIFIED by root: the versioned eight-source manifest binds exact runtime keys using `manual:{dataset}:{sha256(content)}`, pins Central to `masumi-network`, and enforces the exact dataset census `3/3/1/1` across Central, Alice, Bob, and session traces. Focused suite returned `15 passed in 0.09s`; Ruff returned `All checks passed!`; negative proof rejected `central` and a fabricated runtime source key. Blind spot: final-image Docker execution remains pending.
- [x] CITADEL-DOCKER-NONLIVE-FAILURES-01, owner: reviewer. VERIFIED by root: retained-image reproduction returned `3 failed, 1957 passed, 5 deselected, 12 warnings in 32.91s`; failures were a stale timeout assertion, an over-broad `latest` workflow assertion, and missing final `accepted` in the persisted share event. Root integrated bounded fixes. Host non-live suite then returned `1960 passed, 5 deselected, 11 warnings in 28.04s`; Ruff returned `All checks passed!`. Blind spot: corrected final-image tests have not run.
- [x] CITADEL-MESH-TRUTH-01, owner: implementer. VERIFIED static by root: `kb/mesh.py` treats lifecycle evidence as indexed only when `projection_state == "searchable"`; queued or pending accepted writes emit `ingest_pending`, `projection_queued`, and `pending` without document nodes, graph edges, indexed status, or indexed counters. Legacy results without lifecycle evidence retain prior behavior. Ruff returned `All checks passed!`; Python compilation and `git diff --check` returned no output. Blind spot: source-reverted Docker red and focused Docker green remain pending.
- [x] CITADEL-OBSERVED-OUTCOMES-01, owner: reviewer. REPORTED: PR 245 cannot merge wholesale because GitHub reports `mergeStateStatus: DIRTY` and its transient index-state model conflicts with lifecycle v1. Reviewer classified its 34-file diff and reported no P0 plus five current P1 truthfulness gaps: queued mesh writes labeled indexed, readiness treating a never-run canary as passing, capture hooks treating missing acceptance as success, local search dropping provenance basis, and guarded MCP writes omitting claimed confirmation plus promotion refusal detail. No file, Docker resource, or GitHub state changed. Root reproduction remains required before a finding is recorded as VERIFIED.
- [x] CITADEL-OCI-RELEASE-IMPL-01, owner: implementer. VERIFIED static by root: `.github/workflows/publish.yml` stages one `linux/amd64` and `linux/arm64` production index at `ghcr.io/masumi-network/citadel:sha-${GITHUB_SHA}`, emits maximum-mode provenance plus SBOM, smokes exact child digests, creates a keyless GitHub attestation, publishes PyPI only after OCI gates, copies the same index to the unused exact `0.5.0` tag, and creates the GitHub Release last. The workflow publishes no mutable alias. `tests/test_publish_workflow.py` passed Ruff and Python compilation; Ruby parsed the YAML; `git diff --check` returned no output. Blind spot: the workflow has not run, and no registry, PyPI, tag, or release was mutated.
- [x] CITADEL-COMPOSE-SNAPSHOT-01, owner: implementer. VERIFIED static by root: both byte-identical Compose files configure `QDRANT__STORAGE__SNAPSHOTS_PATH=/qdrant/snapshots`, mount distinct `qdrant-data` and `qdrant-snapshots` named volumes, retain the exact Qdrant `1.19.0` digest, and declare both volumes. `.venv/bin/ruff check --no-cache tests/test_docker_workflow.py` returned `All checks passed!`; Python compilation and `git diff --check` returned no output. Blind spot: production Docker snapshot creation and restore remain pending.
- [x] CITADEL-RING0-INTEGRATION-AUDIT-01, owner: reviewer. VERIFIED by root source reproduction: no P0; four P1 release gaps exist. Restore cleanup has an unexecuted lock-race data-loss path, CI classifiers omit warning and generic failure lines, production Compose lacks a read-only root filesystem, and whole-generation restore proof does not boot the shipped production target. Reviewer changed no files. Exact inventory: 22 modified tracked paths, six untracked paths, `git diff --check HEAD` clean.
- [x] CITADEL-HOST-RECOVERY-01, owner: release. REPORTED: the user approved deletion of only `~/.cache/uv`, Docker Desktop restart, and unreferenced BuildKit cache pruning after final3 verification. VERIFIED: the `4,667,272 KiB` UV cache was removed and is recoverable by package redownload. Docker Desktop restarted, final3 returned authenticated readiness `200`, Qdrant retained six collections, and `docker buildx prune --force` removed `3.852GB`. Images remained `15`, containers `23`, volumes `17`, and host free space became `6.3GiB`.
- [x] CITADEL-HOST-RECOVERY-02, owner: release. VERIFIED: explicit approval removed only three rebuildable caches totaling about `2.46GiB`, then Docker Desktop was cleanly restarted. Preserved final3 Citadel returned healthy on image `sha256:c2fdaebc...`; Qdrant returned running on image `sha256:057ee3a8...` with six authenticated collections. BuildKit-only pruning removed `10.62GB`; post-check returned `0B` cache and `10GiB` host free. Images, containers, volumes, repositories, and application data were retained.
- [x] CITADEL-GITHUB-AUDIT-01, owner: release. VERIFIED: root live reads returned 19 open issues and 16 open pull requests, all without milestones. REPORTED by the release auditor: PR 256 is the sole v0.5 integration candidate; issues 46, 104, 106, 124, 128, 135, 228, 247, and 249 remain v0.5 blockers; no issue should close; PRs 246, 254, and 255 are closure candidates only after replacement evidence is visible remotely. No GitHub write occurred.
- [x] CITADEL-BACKUP-GEN-01, owner: implementer. VERIFIED: Docker wrong-generation regression failed before the guard, then generation-bound restore, partial-failure rollback, and count-mismatch rollback returned `3 passed, 1 warning in 0.68s`. A real uploaded Qdrant snapshot count-mismatch regression failed before rollback with one orphaned collection, then returned `1 passed, 5 warnings in 0.87s`; provider logs showed recovery at 100 percent, deletion HTTP `200`, and an empty final collection list. Blind spots: successful whole-generation boot and retrieval, destination containment, private artifact permissions, base-install CLI errors, and authenticated manifest integrity remain separate gates.
...
- [x] CITADEL-BACKUP-CONTAINMENT-01, owner: implementer. VERIFIED: Docker red returned `6 failed, 8 deselected in 1.40s`; the bounded path, mode, cleanup, and dependency fixes returned `15 passed in 1.47s`. Real adapter plus Cognee returned `2 passed, 11 warnings in 75.49s`; successful whole-generation backup and retrieval returned `1 passed, 2 warnings in 75.60s`; count-mismatch rollback returned `1 passed, 4 warnings in 1.55s` and left zero rollback collections. Blind spot: adjacent SHA-256 provides accidental-corruption detection, not authenticated integrity.
- [x] CITADEL-BACKUP-LIVE-ISOLATION-01, owner: runtime. CORRECTED: the first combined live run returned `1 failed, 1 passed` because the successful restore intentionally left six collections in a target reused by the rollback test. No deletion code was added. VERIFIED: CI and local runtime now use separate successful-restore and rollback-restore Qdrant targets. Success retained six collections; rollback started and ended at zero; three provider logs contained zero severe and zero non-2xx lines.

## In Progress
- [ ] CITADEL-CAPTURE-RECEIPT-TRUTH-01, owner: implementer, next action: require explicit `accepted: true` for capture success and write one bounded typed receipt for every requested-capture skip or error path. File scope: `kb/hooks/sync_push.py`, `kb/hooks/sync_session.py`, and focused hook tests. Dependency: CITADEL-OBSERVED-OUTCOMES-01.
- [ ] CITADEL-CURRENT-HEAD-EVIDENCE-01, owner: implementer, next action: run source-reverted Docker red and focused Docker green. VERIFIED source and root static evidence: one SQLite snapshot binds each exact requested dataset/source key to its active non-tombstone head, exact current projection identity, one job, and ordered searchable relational/vector/graph receipts; diagnostic errors remain bounded; historical and cross-wired foreign revisions cannot satisfy release evidence. Root Ruff returned `All checks passed!`; Python compilation and `git diff --check` returned no output. Files: `kb/lifecycle.py`, `kb/service.py`, `tests/test_current_head_evidence.py`. Contract: CITADEL-INT-LIFECYCLE-01.
- [ ] CITADEL-RETRIEVAL-ERRORS-01, owner: implementer, next action: run source-reverted Docker reds and focused plus live Docker greens. VERIFIED source and root static evidence: genuine empty remains successful; provider outage maps to HTTP `503 QDRANT_UNAVAILABLE`; budget expiry maps to HTTP `504 SEARCH_TIMEOUT`; CLI exits nonzero; MCP and status preserve stable code/status; timeout exits before drilldown or telemetry but records failure audit. Root Ruff returned `All checks passed!`; Python compilation and `git diff --check` returned no output. Contract: CITADEL-INT-RETRIEVAL-01.
- [ ] CITADEL-RECEIPT-EXPOSURE-01, owner: implementer, next action: run source-reverted Docker reds and focused Docker greens. VERIFIED source and root static evidence: successful and partial-failure Shared Session Trace responses expose one operation row per final target outcome; retry success reports final acceptance instead of stale initial state; capture JSON copies only lifecycle fields actually returned by the server; MCP preserves response operations. Root Ruff returned `All checks passed!`; Python compilation and `git diff --check` returned no output. Files: `kb/server.py`, `kb/cli.py`, `tests/test_server.py`, `tests/test_capture.py`, `tests/test_mcp_server.py`. Contract: CITADEL-INT-LIFECYCLE-01.
- [ ] CITADEL-LADYBUG-DOCKER-GATE-01, runtime, derive deterministic wheel hash twice, prove source-reverted red, then run focused, full, production Compose, restart, filesystem, and classified-log gates
- [ ] CITADEL-OCI-RELEASE-DESIGN-01, reviewer, specify the missing immutable GHCR image publication contract and supply-chain gates without writes
- [ ] CITADEL-BACKUP-RESTORE-LOCK-01, owner: implementer, next action: run source-reverted Docker red plus focused green. VERIFIED source and static evidence: restore cleanup now begins only after the restore owns the Lite lock; deterministic test preserves winner-owned state; Ruff returned `All checks passed!`. File scope: `kb/generation_backup.py` and `tests/test_generation_backup.py`. Linked blocker: BLK-2026-08-09-07.
- [ ] CITADEL-DOCKER-LOG-ROOTFS-01, owner: implementer, next action: run source-reverted structural Docker red, current green, then fresh production Compose. VERIFIED source and static evidence: classifiers detect warnings and generic failures with a declared Qdrant TLS exception; both identical Compose files set `read_only: true`; Python static checks and `git diff --check` passed. Linked blockers: BLK-2026-08-09-08 and BLK-2026-08-09-09.
- [ ] CITADEL-COMPOSE-READYZ-MODEL-01, owner: implementer, next action: run source-reverted Docker red, current focused green, then fresh production Compose. VERIFIED root cause: startup ingested a real `session-traces` marker, lifecycle projected it, and Cognee called OpenRouter. Current source provisions the required dataset row and ACL directly through `ensure_dataset`; DEC-2026-08-09-03 records the boundary. Static Ruff, compile, and diff checks passed. Linked blocker: BLK-2026-08-09-06.
- [ ] CITADEL-COMPOSE-LLM-PREFLIGHT-01, owner: implementer, next action: wait for CITADEL-COMPOSE-READYZ-MODEL-01, then rerun structural plus production Compose readiness with strict logs after the traced fix. VERIFIED: structural Docker tests returned `2 passed in 0.03s`; the recovered Compose boot still invoked OpenRouter and remained unready. File scope: both Compose files, `.env.lite.example`, workflow runtime environment, and focused Docker tests. Linked blocker: BLK-2026-08-09-06.
- [ ] CITADEL-DOCKER-NONROOT-COGNEE-01, owner: implementer, next action: include accepted changes in the rebuilt candidate. VERIFIED: Lite binds `CACHE_ROOT_DIRECTORY` below its writable root, preserves explicit override, and focused read-only UID `10001` Docker verification returned `10 passed, 11 warnings in 4.77s`. File scope: `/private/tmp/citadel-v050-qdrant/kb/lite_runtime.py`, `tests/test_lite_runtime.py`, and focused `tests/test_cognee_client.py`.
- [ ] CITADEL-STRESS-429-01, owner: implementer, next action: produce the missing source-reverted red proof before integration. VERIFIED: current Docker focused suite returned `4 passed in 0.06s`; all-429 fails and mixed exact-hit plus 429 succeeds. Blind spot: strict pre-fix red proof remains missing. File scope: `/private/tmp/citadel-v050-qdrant/scripts/stress_qdrant_search.py` and `tests/test_stress_qdrant_search.py`.
- [ ] CITADEL-DOCKER-TEST-01, owner: runtime, next action: resolve BLK-2026-08-10-03, then run corrected image Phase 1, live Qdrant Phase 2, and production Compose Phase 3. VERIFIED: corrected test image `sha256:7de749c08a5ab6d760a66652f1e0e5f35b5e8f7e7f702be58a5b63841c2e6d39` built in `1m 31s`; host free space fell below the declared `5GiB` floor before tests started. Protected containers and all volumes remain untouched.
- [ ] CITADEL-SEARCH-EMPTY-01, owner: implementer, next action: rebuild the production image and prove empty-dataset `/search` returns HTTP `200` with zero results while permission and provider failures remain visible. CORRECTED: the existing status canary intentionally treats zero hits as unavailable; acceptance requires honest count `0` and top-level health, not changing that canary contract. File scope: `/private/tmp/citadel-v050-qdrant/kb/cognee_client.py` and focused tests only. Dependency: none. Linked blocker: BLK-2026-08-09-03.
- [ ] CITADEL-QDRANT-2026-08-09-03, owner: release, next action: confirm remote CI at head `3ce3328`, rebuild the final image at that head, then finish final-image Docker verification before marking draft PR 256 ready. CORRECTED 2026-08-10: BLK-2026-08-10-03 through BLK-2026-08-10-06 are Completed; see `agents/blockers.md` and the remediation daily-log entry. VERIFIED: the candidate worktree is clean at remote head `667eae5`; commits `873b6b7`, `eb56c3b`, and `667eae5` split runtime evidence, Docker hardening, and release gates. Ruff returned `All checks passed!`; host non-live pytest returned `1960 passed, 5 deselected, 11 warnings in 43.87s`. GitHub then reported three exact blocking lanes: missing DCO trailers on the three new commits, denied `GPL-3.0-or-later` for Ladybug `0.18.2`, and a gitleaks finding for the documented `CHANGE_ME_WITH_AT_LEAST_32_RANDOM_CHARACTERS` placeholder in signed commit `92ce11a`. Blind spot: corrected final-image tests, live Qdrant, production Compose, and Railway deployment have not run.
- [ ] CITADEL-BACKUP-2026-08-09-01, owner: implementer, next action: rerun the two-Qdrant downloaded-artifact restore and restored-image boot inside the Docker test harness. VERIFIED: `kb/generation_backup.py` now inventories generation collections, seals artifacts, requires empty restore targets, verifies counts and digests, and rolls back partial Qdrant restore. Earlier focused live result: `1 passed, 2 warnings in 52.63s`. Blind spot: this result predates the Docker-only execution rule and the final independent audit.
- [ ] CITADEL-BENCH-2026-08-09-01, owner: release, next action: run current-source Compose capture, ingest, search, MCP trace, concurrency, lock, restart, outage, restore, resource, and frozen retrieval benchmarks under continuous Citadel and Qdrant log capture. VERIFIED: an uncommitted exact-marker harness exists. Blind spot: final exact-image stress and frozen 105-question output remain absent.

## Blocked
- [ ] BLK-2026-08-10-01, owner: implementer, severity: High, next escalation: bind Ladybug mutable extension state below `/data` and rerun production Compose under read-only root. Evidence: `agents/blockers.md`.
- [ ] BLK-2026-08-09-09, owner: implementer, severity: High, next escalation: enforce and Docker-prove a read-only Citadel root filesystem with writable `/data` and `/tmp`. Evidence: `agents/blockers.md`.
- [ ] BLK-2026-08-09-08, owner: implementer, severity: High, next escalation: make CI log classification catch unexpected warning and generic failure lines without rejecting declared phase evidence. Evidence: `agents/blockers.md`.
- [ ] BLK-2026-08-09-07, owner: implementer, severity: High, next escalation: add and pass a concurrent restore lock-race regression that preserves another process's state. Evidence: `agents/blockers.md`.
- [ ] BLK-2026-08-09-06, owner: implementer, severity: High, next escalation: trace and regression-test the post-startup OpenRouter call that keeps a clean production Compose boot unready. Evidence: `agents/blockers.md` and `/private/tmp/citadel-ring0-recovered-app-20260809.log`.
- [ ] CITADEL-GITHUB-SMOKE-01, owner: runtime, severity: Medium, next escalation: obtain explicit current-message approval to stop the preserved local app briefly and use the existing `gh` credential for authenticated GitHub repository reads, then run bounded dry-runs and conditional local Citadel sync. VERIFIED: the Railway credential-export path and the authenticated Docker execution path were both stopped before execution by approval guards. No Railway secret was read, the app was never stopped, no authenticated GitHub request ran, and no local data changed. Temporary GitHub token files were deleted; `test ! -e` passed. The app remains healthy and Qdrant remains running.
...
- [ ] BLK-2026-08-09-03, owner: implementer, severity: High, next escalation: make an empty or missing configured dataset return an honest empty search result, then prove `citadel status --check-search` inside Docker. Evidence: `agents/blockers.md`.
- [ ] BLK-2026-08-09-04, owner: release, severity: High, next escalation: replace the incomplete raw-run CI smoke with a Docker test target plus production Compose black-box gate. Evidence: `agents/blockers.md`.

## Next priorities
- [ ] CITADEL-DOCKER-TEST-01, owner: implementer, exit criteria: after the revised Docker release design is approved, non-live pytest, Ruff, and three live Qdrant contract files run inside the Docker test target with nonzero collection and zero failures.
- [ ] CITADEL-BACKUP-CONTAINMENT-01, owner: implementer, exit criteria: after the revised Docker release design is approved, Docker regressions prove symlink and nested destinations fail before writes, base-install CLI dependency failure is typed, and backup directories and files use private modes.
- [ ] CITADEL-QDRANT-2026-08-09-03, owner: release, exit criteria: current working tree is reviewed, Docker-only focused and full tests pass, fresh-eyes and red-team reviews pass, accepted changes are committed, remote PR 256 contains them after explicit approval, and current-head CI passes.
...
### 2026-08-10
- Owner: coordinator
- Completed: corrected eight-source release probe, diagnosed first image's three non-live failures, integrated bounded fixes, and committed them by feature as `873b6b7`, `eb56c3b`, and `667eae5`. VERIFIED after commit: Ruff returned `All checks passed!`; host non-live pytest returned `1960 passed, 5 deselected, 11 warnings in 43.87s`; push advanced draft PR 256 from `420be9d` to `667eae5`. Corrected test image digest is `sha256:7de749c08a5ab6d760a66652f1e0e5f35b5e8f7e7f702be58a5b63841c2e6d39`.
- In Progress: resolve remote PR blockers, then final-image Phase 1, live Qdrant Phase 2, production Compose Phase 3, independent reviews, and the conditionally approved Railway deployment.
- Blocked: BLK-2026-08-10-03 through BLK-2026-08-10-06. Host capacity blocks Docker. PR 256 is blocked by three missing DCO trailers, Ladybug's denied GPL license, and an example placeholder caught by gitleaks. PR 255 also contains five unsigned local commits and will require the same history-rewrite approval. Railway target remains ambiguous between active `Citadel` without Qdrant and scheduled-deletion `Citadel (qdrant)` with Qdrant.
- Next: start a fresh task from this handoff. Resolve the Ladybug distribution decision first. Then fix the gitleaks example, obtain approval for exact force-with-lease DCO rewrites, prune only BuildKit cache after approval, and resume the existing corrected image without rebuilding it.
...
### 2026-08-09
- Owner: coordinator. Completed: root reproduced all four Ring 0 audit P1 findings and settled DEC-2026-08-09-03. In Progress: source-reverted Docker reds and current greens for dataset provisioning, restore lock ownership, log classification, and read-only root. Blocked: exact production restore boot remains under BLK-2026-08-09-02. Next: fresh production Compose readiness, then deterministic seed and GitHub sync.

### 2026-08-10
- Owner: coordinator.
- Completed: source-reverted Docker reds failed as expected; current focused and full non-live gates passed; OpenRouter startup calls were eliminated.
- In Progress: CITADEL-LADYBUG-WRITABLE-HOME-01.
- Blocked: BLK-2026-08-10-01. Ladybug extension installation targets the read-only root and keeps `/readyz` at HTTP `503`.
- Next: bind supported Ladybug mutable state below `/data`, rerun production Compose, then start deterministic seed and GitHub sync.

### 2026-08-10 (PR 256 CI remediation session)
- Owner: coordinator
- Context: a host reboot cleared `/private/tmp`, deleting the candidate worktree and venv and stopping every container. VERIFIED: the branch survived; the worktree was recreated at the same path from local `agent/citadel-v050-qdrant`, and `df` returned `18Gi` free, which retired the BuildKit capacity blocker without a prune. CI parity required `uv sync --locked --all-extras --dev` plus the audited Cognee wheel reinstall; the wheel rebuilt to the exact recorded SHA-256 `890a5a5c7d4b...`, confirming the deterministic builder.
- Completed: live PR 256 triage found five failing lanes, two of them absent from the previous checkpoint: CodeQL with two high `py/overly-permissive-file` alerts (`kb/local_deploy.py:79`, `tests/test_local_deploy.py:154`) and `pytest + ruff (3.12)` failing `test_failed_acknowledgement_retries_without_external_activity` with lease-expiry retries. All five were remediated and pushed as head `3ce3328` after review.
- VERIFIED remediation evidence: full suite `1963 passed, 5 skipped, 11 warnings in 41.32s`; Ruff `All checks passed!`; deflaked test 10 of 10 green plus a negative control where removing only the clock jump fails the test; gitleaks `8.30.1` red-green matrix over `dir` and `git origin/main..HEAD` scans including two planted-key negative controls; ladybug pin test fails on a simulated `0.18.3` bump and passes at `0.18.2`.
- VERIFIED license correction: the Ladybug GPL denial is a metadata misdetection. PyPI `ladybug` 0.18.2 metadata and the LadybugDB/ladybug repository both declare MIT. The workflow now carries a name-scoped `allow-dependencies-licenses` exemption with a pin-equality compensating test, because the action's `purlsMatch()` ignores purl versions.
- REPORTED adversarial gate: a six-agent workflow (four refute lenses, two verifiers, zero errors) reviewed the diff before push and confirmed two findings that were fixed pre-push: the 0o640 fixture literal would have left CodeQL alert 191 open (verified against the live code-scanning API and the QL source), and the original exemption comment claimed a version-pinned control the action does not implement.
- VERIFIED DCO closure: PR 256 commits `873b6b7`, `eb56c3b`, `667eae5` rewritten as `428e44e`, `f25f94c`, `fc06833` with per-commit tree equality; PR 255 branch rewritten with final tree `7e6b86fa` unchanged; PR 256 pushed with `--force-with-lease` against verified remote head `667eae5`.
- Blocked: BLK-2026-08-10-01 remains the production Compose readiness blocker and is untouched by this session.
- Next: watch remote CI at head `3ce3328`; if green, rebuild the final image at the new head and run Phase 1, live Qdrant Phase 2, and production Compose Phase 3 from the indexed handoff, noting the old corrected image binds superseded commit `667eae5`.
- Owner: coordinator. Completed: host recovery and recovered structural image proof. In Progress: CITADEL-COMPOSE-READYZ-MODEL-01 and Ring 0 integration audit. Blocked: BLK-2026-08-09-06 after production Compose reached Uvicorn but repeated `/readyz` HTTP `503` following an unauthenticated OpenRouter call. Next: trace caller, add focused Docker regression, rerun clean Compose readiness, then proceed to deterministic seed and GitHub sync.
- Owner: coordinator
...
- Owner: coordinator
- CORRECTED: local HEAD remains `5bdcf89`, but the implementation worktree now has 17 modified tracked paths and six untracked paths. Earlier green results for `5bdcf89` do not attest this delta.
- VERIFIED: exact Docker image `sha256:c2fdaebc720e22eb5926c8fd15e5f900d22ea46244ac3f6d99756eba87877cae` booted healthy. Auth returned expected `401`, `403`, and `200`; populated lifecycle search returned the exact marker; corpus returned four documents; all three lifecycle receipts were searchable.
- Blocked: BLK-2026-08-09-03 records missing default dataset search as HTTP `500`. BLK-2026-08-09-04 records the incomplete CI Docker gate. BLK-2026-08-09-02 now includes final backup review findings.
- Next: resume from `.local-review/handoffs/docker-qdrant-release-2026-08-09.md` in a fresh root session using the orchestration-first rules in `AGENTS.md` and `agents/agents.md`.
- VERIFIED: task-to-model routing, reasoning effort, dependencies, linked blockers, scopes, and acceptance commands are indexed in `agents/model-routing.md`.
- Owner: release
- Completed: CITADEL-HOST-RECOVERY-01. UV and BuildKit caches were removed within the approved scope. Final3 app and Qdrant resources remained healthy with six collections. Host free space is `6.3GiB`.
- In Progress: CITADEL-COMPOSE-LLM-PREFLIGHT-01 and CITADEL-DOCKER-TEST-01. Current source needs a rebuild and strict production Compose readiness gate.
- Blocked: authenticated GitHub sync still requires its separate current-message service-interruption and private-repository-read approval.
- Next: transfer Docker ownership to the indexed runtime operator, rebuild current source, and run structural plus Compose readiness gates with both logs followed.
...
## Citadel v0.5 Docker release design gate: 2026-08-09

Owner: coordinator
Status: In Progress

- REPORTED: the user selected a local Docker release proof ordered around GitHub sync, graph construction, capture, ingest, CLI, MCP, direct API, deterministic seed evidence, benchmarks, session traces, seat isolation, promotion, and only then a portable v0.5 Docker publication.
- REPORTED: the user selected deterministic seed acceptance first, followed by live GitHub sync through the same Docker gates.
- REPORTED: the user selected an allowlisted Railway variable import first. Expand beyond the allowlist only when a Docker gate proves a required value is missing; do not mirror the full production environment by default.
- REPORTED: the user selected quality plus relative-latency benchmark gates. Docker is the local reproducibility and production-architecture proof; it is not expected to improve retrieval quality. Keep the current model only when the frozen same-corpus quality gates pass. A quality failure blocks v0.5 for a stronger-model comparison; Docker latency alone does not trigger a model replacement.
- VERIFIED: `superpowers:brainstorming` was invoked for this plan. Its hard gate paused CITADEL-DOCKER-TEST-01 and CITADEL-BACKUP-CONTAINMENT-01 before further implementation.
- REPORTED: the user approved design section 1. The accepted sequence is immutable local release candidate, deterministic seed proof, live GitHub proof through the same gates, quality and isolation gates, independent reviews, PR integration, then a separate publication approval.
- REPORTED: the user then approved all remaining design sections and requested the active goal be updated.
- VERIFIED: preserved release containers remain outside this design change. No Railway or GitHub write, deployment, push, merge, tag, publication, production mutation, or production deletion occurred.
- VERIFIED: while the design gate remained pending, `docker ps` showed `citadel-final-image-app-final3-20260809` healthy and `citadel-service-e2e-qdrant-20260809` running. A 15-minute log classification found Citadel `42` request lines and Qdrant `504` request lines, with zero non-2xx requests and zero severe lines in either log. Blind spot: this was passive health traffic, not a fresh functional gate.
- VERIFIED: read-only reinspection found the implementation worktree still has `17` modified tracked paths and `6` untracked paths. `tests/test_docker_workflow.py` requires a Docker `test` target, non-root `USER 10001:10001`, three live Qdrant suites, non-live collection, Ruff, and log followers. The current workflow still defines `container-smoke`, omits `CITADEL_QDRANT_SERVER_IMAGE`, and disables lifecycle. No source was changed during this inspection.
- CORRECTED: the earlier no-push guard is superseded for the approved PR lane only. Root may update PR 256 and perform evidence-backed tracker maintenance after local Docker gates, independent reviews, and sequential commits pass. Merge, tag, publication, deployment, Railway mutation, production mutation, and production data deletion remain forbidden without another approval.
- VERIFIED: the approved design was written to `docs/superpowers/specs/2026-08-09-citadel-v050-docker-release-design.md`. `git diff --check` returned no output and the manual banned-writing scan returned no matches.
- Next: let the user review the written design, invoke `superpowers:writing-plans`, then resume the indexed implementation tasks.
