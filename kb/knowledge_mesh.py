...
This is intentionally separate from :class:`kb.vault_activity.MeshState`, which
is the live Vault Activity dashboard projection of runtime operations (not the
mesh). The Knowledge Mesh exposes what the Organization Vault actually knows:
nodes and relationships read from Cognee's graph engine (Kuzu in the v1
deployment).
