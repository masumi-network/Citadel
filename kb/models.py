...
def worst_index_state(states: Iterable[str]) -> str:
    """The least-indexed state in ``states``, or ``unknown`` when there are none.

    A document written as several chunks is findable only if all of them are,
    so the document's state is the worst of its parts, never the best — the
    opposite choice would let one indexed chunk vouch for a document whose rest
    never reached the graph. Lives here because more than one surface has to
    apply the same pessimism, and two copies of a rule drift.
    """
    ranked = list(states)
    if not ranked:
        return INDEX_STATE_UNKNOWN
    # False (observed not indexed) beats None (unobserved) beats True.
    return min(ranked, key=lambda state: {False: 0, None: 1, True: 2}[index_flag(state)])
