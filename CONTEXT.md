...
**Node**:
A seat's mini knowledge base; logically isolated storage for that seat's agent memory. **Nodes** do not collide. Seats are not intended to read each other's **Nodes**; ADR-0021 proposes changing that intent deliberately for `org-work` content while keeping `personal`-tagged capture roots private. Writes never cross **Nodes** under either reading (ADR-0007).
...
- A **Session Trace** is private to its **Seat**'s **Node** by default (see **Node** for the visibility change ADR-0021 proposes); a **Shared Session Trace** is a volunteered copy in `session-traces` readable by every **Seat** via **`citadel_search`**, never **Central** and never another seat's **Node**.
...
- "can the evolve pipeline run in its own process?" (2026-07-29, #88); resolved: **no**. The retrieval engine opens the **Knowledge Mesh** graph with an exclusive OS file lock held for the lifetime of whichever process opens it, and there is no working read-only mode. A second process cannot open the graph at all, for reads or writes, so **one process owns the graph** and every graph-touching stage runs inside it (ADR-0015). This is why the evolve subprocess could never succeed, why the loop-binding workaround became unnecessary, why a second uvicorn worker is not a configuration option, and why a separate evolve cron service sharing `/data` is wrong even though it looks reasonable. The constraint belongs to the engine, not to Citadel: it is the sharpest argument for ADR-0010, since owning **Structured Knowledge** is what would let the engine be swapped for one that permits concurrent openers.
- "does a green stage mean the work happened?" (2026-07-29); resolved: **no, and that gap is a recurring defect class rather than a set of unrelated bugs**. Six independent surfaces reported success for work that did not occur, in one day: an evolve pass exiting 0 with two dead stages, **Promotion** reporting zero failures while every seat's search died, the cognify canary passing on graph growth instead of retrievability, a scheduler task death swallowed by its done-callback, an unconfigured delivery gateway making both the contact endpoint and the **Organization Update Digest** do nothing, and a timed-out search recorded as a successful one. The shape is always the same: ordinary failure paths are handled carefully, while the "nothing was configured" and "everything failed" paths fall through to the success branch. A **Learning Process** stage, a delivery, or a canary must report what it actually did, and an empty result caused by failure must be distinguishable from an empty result caused by there being nothing to do.
- "is Postgres or qdrant the better database?" (2026-08-04 grill) conflated two roles Postgres plays; resolved: Postgres is both the relational store and the current **Knowledge Index** backend, and only the second is replaceable. The subject of any such comparison is the **Knowledge Index** backend, never "the database" — a swap leaves Postgres running.
- "does the Knowledge Index approximate?" (2026-08-04 grill); resolved: **no**. The current backend performs an exact scan, so recall is total and latency grows linearly with the corpus. This was inherited from a default rather than chosen, and it is the opposite of what a reader assumes. Related: the Index is given no filter of its own, so dataset scoping is resolved above it rather than inside the scan (ADR-0020, ADR-0009).
- "is a search recorded?" (2026-08-04 grill); resolved: **yes, and by the engine rather than by us**. Citadel's own telemetry deliberately keeps no query text and is ephemeral, so **Vault Activity** stays presence-and-timing only as designed. But the retrieval engine durably stores every query string with its caller in the relational store, on by default, which is where a real query set comes from and which is an unowned privacy surface. First resolved the opposite way in this same session by reading only Citadel's code: the lesson is that a claim about *system* behaviour cannot be established from our layer alone when a vendored engine sits underneath it (ADR-0020).
- "should seats be able to read each other's **Nodes**?" (2026-08-04) is OPEN. The glossary above records the intent the rest of this vocabulary is built on: no. ADR-0021 proposes the reverse, that `org-work` **Node** content becomes readable by every **Seat** while `personal`-tagged capture roots stay private, with the write path and **Promotion** unchanged. It is **Proposed, not Accepted**: consent from every current seat holder is a stated precondition, the private partition it depends on does not exist, and the post-flip search arity is unsettled. If and when it is accepted, three statements in this file (the **Node** definition, the **Session Trace** entry, and the **Relationships** line on trace visibility) and ADR-0011's "reads never cross seat **Nodes** holds literally and without exception" are all superseded and must be revised in the same change. Until then this entry is the only record that the question is open.
