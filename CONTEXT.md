...
- "does the Knowledge Index approximate?" (2026-08-04 grill); resolved: **no**. The current backend performs an exact scan, so recall is total and latency grows linearly with the corpus. This was inherited from a default rather than chosen, and it is the opposite of what a reader assumes. Related: the Index is given no filter, so it enforces no isolation of its own — dataset scoping is resolved above it (ADR-0020, ADR-0009).
...
- "can seats read each other's **Nodes**?" (2026-08-04) is OPEN, and the glossary above still carries the answer that holds today: no. ADR-0021 proposes the reverse, that `org-work` **Node** content becomes readable by every **Seat** while `personal`-tagged capture roots stay private, with the write path and **Promotion** unchanged. It is **Proposed, not Accepted**: consent from every current seat holder is a stated precondition, the private partition it depends on does not exist, and the post-flip search arity is unsettled. If and when it is accepted, three statements in this file (the **Node** definition, the **Session Trace** entry, and the **Relationships** line on trace visibility) and ADR-0011's "reads never cross seat **Nodes** holds literally and without exception" all become false and must be revised in the same change. Until then this entry is the only record that the question is open.
