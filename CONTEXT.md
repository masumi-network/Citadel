...
Install once: `citadel onboard` (idempotent). It writes a self-contained git pre-push hook (`python -m kb.hooks.sync_push`), SessionEnd/SessionStart hooks (`kb.hooks.sync_session` / `kb.hooks.sync_start`; SessionStart may inject bounded repo-scoped workspace candidates), the proactive agent policy (`AGENTS.md` plus native rules for Cursor, Windsurf, and Gemini when detected), project `.mcp.json`, and detected client MCP configs (Claude Code, Cursor, Codex, Gemini, Windsurf). On macOS it also runs `launchctl setenv` so Dock-launched Cursor can see `CITADEL_MCP_ACCESS_TOKEN` until logout.
